"""
Сервис валидации DataFrame по схеме DATA_SCHEMA.

Проверки:
    1. DataFrame не пустой.
    2. Все обязательные колонки присутствуют.
    3. Числовые колонки приводятся к числу без ошибок.
    4. Категориальные колонки содержат только допустимые значения.

Расширение: добавить новый тип проверки -> создать метод _check_* и вызвать в validate().
"""
...
class ValidationResult:
    """Агрегированный результат валидации"""
...
class DataValidationService:
    """
    Валидация DataFrame по DATA_SCHEMA.
    """
    ...
    def _check_missing_columns(df: pd.DataFrame) -> list[str]:
        """Возврат список отсутствующих обязательных колонок."""
    ...
    def _check_numeric_columns(df: pd.DataFrame) -> list[FieldError]:
        """Проверка числовых колонок: попытка приведения к числу."""
        errors: list[FieldError] = []
    ...
    def _check_categorical_columns(df: pd.DataFrame) -> list[FieldError]:
        """Проверка категориальных колонок на допустимые значения."""
        errors: list[FieldError] = []
        for spec in DATA_SCHEMA:
