...
MSG_UNSUPPORTED_MODEL_FILE: str = (
    "Формат файла модели не поддерживается. "
    "Допустимые форматы: {extensions}"
)
MSG_UNSUPPORTED_DATA_FILE: str = (
    "Формат файла данных не поддерживается. "
    "Допустимые форматы: {extensions}"
)
MSG_NO_DATA: str = (
    "Данные для предсказания не переданы. "
    "Загрузите CSV-файл или передайте form_data."
)
...
MSG_DEFAULT_MODEL_NOT_FOUND: str = (
    "Модель по умолчанию не найдена: {path}. "
    "Загрузите файл модели или поместите model.pkl в директорию ./models."
)
MSG_PREDICTION_SUCCESS: str = (
    "Предсказание выполнено успешно. Обработано строк: {count}."
)
...
MSG_EMPTY_DATAFRAME: str = (
    "Переданные данные не содержат ни одной строки."
)
MSG_MISSING_COLUMNS: str = (
    "Отсутствуют обязательные колонки: {columns}"
)
MSG_INVALID_DTYPE: str = (
    "Колонка '{column}' должна иметь тип {expected}, "
    "но содержит нечисловые значения: {values}"
)
MSG_INVALID_CATEGORY: str = (
    "Колонка '{column}' содержит недопустимые значения: {invalid}. "
    "Разрешённые значения: {allowed}"
)
